...
@login_required(login_url='teaherlogin')
@user_passes_test(is_teacher)
def delete_student_teacher_from_school_view(request,pk):
    student=models.StudentExtra.objects.get(id=pk)
    user=models.User.objects.get(id=student.user_id)
    user.delete()
    student.delete()
...
@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def delete_student_teacher_view(request,pk):
    student=models.StudentExtra.objects.get(id=pk)
    user=models.User.objects.get(id=student.user_id)
    user.delete()
    student.delete()
...
@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def update_student_teacher_view(request,pk):
    student=models.StudentExtra.objects.get(id=pk)
    user=models.User.objects.get(id=student.user_id)
    form1=forms.StudentUserForm(instance=user)
    form2=forms.StudentExtraForm(instance=student)
...
def student_message_view(request):
    studentdata=models.StudentExtra.objects.all().filter(status=True,user_id=request.user.id)
    ...
    if request.method=='POST':
        ...
        if feedback.is_valid():
            ...
            print("form is invalid")
        return render(request,'school/feedback_sent.html',{'student':studentdata})
    return render(request,'school/student_feedback.html',{'feedback':feedback,'student':studentdata})

def teacher_view_student_feedback_view(request):
    feedback=models.Feedback.objects.all().order_by('-id')
    return render(request,'school/teacher_feedback.html',{'feedback':feedback})
